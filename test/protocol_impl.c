...
struct CompostSliceU8 caesar_cipher_handler(struct CompostCtx *ctx, struct CompostSliceU8 str, uint8_t offset)
{
    struct CompostSliceU8 ciphertext = compost_slice_u8_new(&ctx->alloc, str.len);
    for (int i = 0; i < str.len; i++)
        ciphertext.ptr[i] = str.ptr[i] + offset;
    return ciphertext;
}

struct CompostSliceU8 sort_bytes_handler(struct CompostCtx *ctx, struct CompostSliceU8 data)
{
    struct CompostSliceU8 sorted = compost_slice_u8_new(&ctx->alloc, data.len);
...
struct ListFirstAttr list_first_attr_handler(struct CompostCtx *ctx, struct CompostSliceI16 data)
{
    struct ListFirstAttr ret = ListFirstAttr_init(&ctx->alloc, data.len);
...
struct ListMidAttr list_mid_attr_handler(struct CompostCtx *ctx, struct CompostSliceI16 data)
{
    struct ListMidAttr ret = ListMidAttr_init(&ctx->alloc, data.len);
    compost_slice_copy(ret.data, data);
    ret.min = slice_min(ret.data);
    ret.max = slice_max(ret.data);
    return ret;
...
struct ListLastAttr list_last_attr_handler(struct CompostCtx *ctx, struct CompostSliceI16 data)
{
    struct ListLastAttr ret = ListLastAttr_init(&ctx->alloc, data.len);
    compost_slice_copy(ret.data, data);
    ret.min = slice_min(ret.data);
    ret.max = slice_max(ret.data);
    return ret;
...
struct TwoListAttr two_list_attr_handler(struct CompostCtx *ctx, struct CompostSliceI16 data_a,
                                     struct CompostSliceI16 data_b)
{
    struct TwoListAttr ret = TwoListAttr_init(&ctx->alloc, data_a.len, data_b.len);
...
struct MockDate epoch_to_date_handler(struct CompostCtx *ctx, int32_t epoch)
{
    struct MockDate ret = MockDate_init(&ctx->alloc, MOCK_DATE_FMT_LEN, MOCK_DATE_FMT_LEN);
    set_mock_date(&ret, (time_t)epoch);
    return ret;
}

struct CompostSliceU8 emoji_handler(struct CompostCtx *ctx, struct CompostSliceU8 text)
{
    ...
    if (strncmp((char *)text.ptr, "😘", text.len)) {
        ...
        return compost_str_new(&ctx->alloc, "🥰"); // Returning slice inside of the TX buffer
    }
}

struct CompostSliceU32 cat_lists_handler(struct CompostCtx *ctx, struct CompostSliceU32 list_a,
                                       struct CompostSliceU32 list_b)
{
    struct CompostSliceU32 ret = compost_slice_u32_new(&ctx->alloc, list_a.len + list_b.len);
...
struct MockLfsr get_random_number_handler(struct CompostCtx *ctx, uint64_t seed, uint8_t iter)
{
    struct MockLfsr lfsr = MockLfsr_init(&ctx->alloc, MOCK_DATE_FMT_LEN, MOCK_DATE_FMT_LEN);
